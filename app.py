...
def rediscover_devices():
    ...
    link_resources = {
        "_links": {"self": {"href": "/devices/rediscover"}, "all": {"href": "/devices"}}
    }
...
def get_device(device_name):
    ...
    if found_device == None:
        return Response(
            json.dumps({"error": "no device found"}),
            status=404,
        ...
        return Response(
            json.dumps(device), status=200, mimetype="application/json", headers=headers
        )


@app.route("/devices/<string:device_name>/toggle", methods=["PUT", "POST"])
...
@app.route("/devices/<string:device_name>/on", methods=["PUT", "POST"])
...
@app.route("/devices/<string:device_name>/off", methods=["PUT", "POST"])
...
def format_multiple_devices_response(devices, general_resources):
    ...
    if len(devices) == 0:
        return Response(
            json.dumps({"error": "no devices found"}),
            status=404,
            mimetype="application/json",
            headers=headers,
        )
        ...
        complete_devices_response = {
            "count": len(embedded_device_resources),
            **general_resources,
            "_embedded": {"devices": embedded_device_resources},
        }

        return Response(
            json.dumps(complete_devices_response),
            status=200,
            mimetype="application/json",
            headers=headers,
        )
